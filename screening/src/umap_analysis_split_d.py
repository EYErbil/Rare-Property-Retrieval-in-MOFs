#!/usr/bin/env python3
"""
UMAP Analysis for Split D (embedding-informed farthest-point split)
====================================================================

Standalone UMAP visualization of Split D -- the embedding-informed split
that guarantees each test positive has a nearby train positive.

This is the counterpart to umap_analysis_original_split.py, producing
the same set of output files but with Split D as the primary analysis.

Outputs (into --output_dir, default ./umap_split_d/):
  - umap_splitd_bandgap.png          (colored by bandgap, stars for positives)
  - umap_splitd_diagnosis.png        (highlights test positives by coverage)
  - umap_splitd_report.txt           (nearest-neighbor diagnosis table)
  - umap_splitd_analysis_summary.json(machine-readable summary)
  - umap_coords.npz                  (UMAP coordinates, reusable)
  - umap_comparison_d_vs_original.png(side-by-side if --original_splits_dir given)
  - umap_original_report.txt         (NN report for original split, if given)
...
Usage:
  python umap_analysis_split_d.py \\
      ...
      --output_dir ./umap_split_d

  # With comparison to original split:
  python umap_analysis_split_d.py \\
      --embeddings_path ./embedding_analysis/embeddings_pretrained.npz \\
      --splitd_dir ./new_splits/strategy_d_farthest_point \\
...
def plot_bandgap_umap(coords, bandgaps, splits, threshold, output_path,
    ...
    """Panel 1: All points colored by bandgap. Stars for test positives.
       Panel 2: Colored by train/val/test assignment."""
    ...
    ax.set_title(f'UMAP -- Colored by Bandgap (eV)\n{split_label}', fontsize=13)
    ...
    ax.set_title(f'UMAP -- Colored by Split\n{split_label}', fontsize=13)
    ax.legend(fontsize=10, loc='upper right')
...
def plot_diagnosis_umap(coords, bandgaps, splits, cif_ids, nn_results,
                        threshold, output_path, split_label="Split D"):
    """
    ...
    ax.set_title(f'UMAP Diagnosis -- {split_label}\n'
                 'Test positives colored by structural coverage in training set',
                 fontsize=13)
...
def plot_comparison_umap(coords, bandgaps, splits_d, splits_original,
                         cif_ids, threshold, output_path):
    """Side-by-side: Split D vs original split on same UMAP."""
    pos_mask = bandgaps < threshold
    fig, axes = plt.subplots(1, 2, figsize=(22, 9))

    for ax, splits, title in [
        (axes[0], splits_d, 'Split D (farthest-point coverage)'),
        (axes[1], splits_original, 'Original Split (new_subset)')
    ]:
        train_mask = np.array([s == 'train' for s in splits])
        test_mask = np.array([s == 'test' for s in splits])
...
def generate_report(nn_results, output_path, split_name="Split D"):
    """Generate text report (same format as original split report)."""
    lines = []
    W = 100
    ...
    if n_isolated > 0:
        ...
        lines.append(f"     Consider further re-splitting to improve coverage.")
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description='UMAP Analysis for Split D (embedding-informed split)',
...
""")
    ...
    parser.add_argument('--splitd_dir', type=str, required=True,
                        help='Path to strategy_d_farthest_point/ with '
                             '{train,val,test}_bandgaps_regression.json')
    parser.add_argument('--original_splits_dir', type=str, default=None,
                        help='(Optional) Path to new_subset/ for comparison')
    parser.add_argument('--output_dir', type=str, default='./umap_split_d',
    ...
    # =========================================================================
    print("=" * 80)
    print("  UMAP ANALYSIS -- SPLIT D (farthest-point embedding-informed)")
    print("=" * 80)
    ...
    # =========================================================================
    # 2. Load Split D labels
    # =========================================================================
    print(f"\n  Loading Split D labels from: {args.splitd_dir}")
    labels_d, split_assign_d = load_split_labels(args.splitd_dir, args.downstream)
    ...
    bandgaps_override = np.array([
        labels_d.get(cid, bandgaps[i]) for i, cid in enumerate(cif_ids)
    ])

    n_train = sum(1 for s in splits_d if s == 'train')
    n_val = sum(1 for s in splits_d if s == 'val')
    n_test = sum(1 for s in splits_d if s == 'test')
    n_pos = sum(1 for bg in bandgaps_override if bg < args.threshold)
    n_train_pos = sum(1 for i, s in enumerate(splits_d)
                      if s == 'train' and bandgaps_override[i] < args.threshold)
    n_test_pos = sum(1 for i, s in enumerate(splits_d)
    ...
    # =========================================================================
    # 4. Nearest-neighbor diagnosis for Split D
    # =========================================================================
    print(f"\n--- Nearest-Neighbor Diagnosis (Split D) ---")
    ...
    plot_bandgap_umap(coords, bandgaps_override, splits_d, args.threshold,
                      os.path.join(args.output_dir, 'umap_splitd_bandgap.png'),
                      split_label="Split D (farthest-point coverage)")

    plot_diagnosis_umap(coords, bandgaps_override, splits_d, cif_ids,
                        nn_results_d, args.threshold,
                        os.path.join(args.output_dir, 'umap_splitd_diagnosis.png'),
                        split_label="Split D")

    # =========================================================================
    # 6. Comparison with original split (if provided)
    # =========================================================================
    if args.original_splits_dir:
        print(f"\n--- Comparison with Original Split ---")
        _, split_assign_orig = load_split_labels(
        ...
        for cid in cif_ids:
            if cid in split_assign_orig:
                ...
                splits_original.append('unknown')

        n_train_o = sum(1 for s in splits_original if s == 'train')
        n_test_o = sum(1 for s in splits_original if s == 'test')
        n_train_pos_o = sum(1 for i, s in enumerate(splits_original)
                            if s == 'train' and bandgaps_override[i] < args.threshold)
        n_test_pos_o = sum(1 for i, s in enumerate(splits_original)
                           if s == 'test' and bandgaps_override[i] < args.threshold)
        print(f"  Original: Train {n_train_o} ({n_train_pos_o} pos), "
              f"Test {n_test_o} ({n_test_pos_o} pos)")

        plot_comparison_umap(coords, bandgaps_override, splits_d, splits_original,
                             cif_ids, args.threshold,
                             os.path.join(args.output_dir,
                                          'umap_comparison_d_vs_original.png'))

        nn_results_orig = nn_diagnosis(cif_ids, embeddings, bandgaps_override,
                                       splits_original, args.threshold)
        generate_report(nn_results_orig,
                        os.path.join(args.output_dir, 'umap_original_report.txt'),
                        split_name="Original")

    # =========================================================================
    # 7. Text report for Split D
    # =========================================================================
    print(f"\n--- Generating Report ---")
    ...
    # Save JSON summary
    summary = {
        'split': 'Split D (strategy_d_farthest_point)',
        'n_total': len(cif_ids),
        'n_train': n_train,
        'n_test': n_test,
        'n_positive': n_pos,
    ...
    for r in sorted(nn_results_d, key=lambda x: x['pos_neg_gap']):
    ...
    with open(os.path.join(args.output_dir, 'umap_splitd_analysis_summary.json'), 'w') as f:
    ...
    print(f"{'=' * 80}")
    print(f"  Files:")
    print(f"    umap_splitd_bandgap.png             -- UMAP colored by bandgap + splits")
    print(f"    umap_splitd_diagnosis.png            -- Test positives colored by coverage")
    print(f"    umap_splitd_report.txt               -- Full NN diagnosis table")
    print(f"    umap_splitd_analysis_summary.json    -- Machine-readable summary")
    print(f"    umap_coords.npz                      -- UMAP coordinates (reusable)")
    if args.original_splits_dir:
        print(f"    umap_comparison_d_vs_original.png    -- Side-by-side Split D vs original")
        print(f"    umap_original_report.txt             -- NN diagnosis for original split")
