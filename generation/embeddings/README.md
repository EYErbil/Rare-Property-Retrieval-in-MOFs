...
These large inputs are restored from the paper's Globus collection into this folder. They are used
...
| File (after restore) | Globus path | Verified contents |
|---|---|---|
...
uploaded to Globus. Select the 9,561 unlabeled IDs from `pmt_embeddings_qmof_all.npz` instead.
...
descriptor-row order without a separately distributed CSV. Globus also ships the corresponding
