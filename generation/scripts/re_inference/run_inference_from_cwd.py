...
Run from the experiment folder (same place as run.py):
  ...
  python /path/to/rare-property-retrieval/generation/scripts/re_inference/run_inference_from_cwd.py
...
# When script is in experiments/exp364/, project root is parent of experiments/
...
def main():
    parser = argparse.ArgumentParser(
        description="Run NN inference using best checkpoint in current experiment dir."
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        required=True,
        help="Processed-data directory containing inference/test files.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
        help="Where to write CSV/TXT outputs (default: current working directory).",
    )
    parser.add_argument(
        "--top_k",
        type=int,
        default=25,
        help="Number of top structures to write into top{K}_for_DFT.txt (default: 25).",
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        default=None,
        help="Optional explicit checkpoint path. If omitted, auto-detect best_es in current dir.",
    )
    ...
    output_dir = os.path.abspath(args.output_dir) if args.output_dir else exp_dir
    os.makedirs(output_dir, exist_ok=True)

    # Ensure test JSON (prefer test_bandgaps.json)
    test_json = os.path.join(data_dir, "test_bandgaps.json")
    if not os.path.exists(test_json):
        legacy_test = os.path.join(data_dir, "test_bandgaps_regression.json")
        if os.path.exists(legacy_test):
            import shutil
            shutil.copy2(legacy_test, test_json)
        else:
            inf_json = os.path.join(data_dir, "inference_bandgaps_regression.json")
            if not os.path.exists(inf_json):
                raise SystemExit(f"Neither {test_json}, {legacy_test}, nor {inf_json} found")
            import shutil
    ...
    # Top-K
    top_ids = [cif_ids[i] for i in order[: args.top_k]]
    txt = os.path.join(output_dir, f"top{args.top_k}_for_DFT.txt")
