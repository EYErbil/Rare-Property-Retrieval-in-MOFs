...
SOURCE_STAGE="${SOURCE_STAGE:-PBED3-PreRelax}"
TARGET_STAGE="${TARGET_STAGE:-PBED3-Relax}"
SOURCE_MAX_STEP_ITER="${SOURCE_MAX_STEP_ITER:-149}"
TARGET_MAX_STEP_ITER="${TARGET_MAX_STEP_ITER:-249}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INCAR_TEMPLATE="$SCRIPT_DIR/INCAR"

if [ ! -s "$INCAR_TEMPLATE" ]; then
  echo "ERROR: missing relax INCAR template: $INCAR_TEMPLATE"
  exit 1
fi
...
find "$ROOT" -mindepth 1 -maxdepth 1 -type d ! -name "copy" | sort | while IFS= read -r mofdir; do
  ...
  if mof_already_in_queue "$mofname" "$source" "$target"; then
  ...
  copy_required "$source/CONTCAR" "$target/POSCAR"
  ...
  if [ -f "$target/INCAR" ]; then
    cp "$target/INCAR" "$target/INCAR.bak_before_relax_from_prerelax_copy"
  fi

  cp "$INCAR_TEMPLATE" "$target/INCAR"
  echo "  COPIED INCAR TEMPLATE: $INCAR_TEMPLATE -> $target/INCAR"
  copy_magmom_from_source_incar "$source/INCAR" "$target/INCAR"
done
